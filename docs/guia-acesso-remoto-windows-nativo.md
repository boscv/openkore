...
Use o script pronto: `tools/start-gateway.ps1`.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\start-gateway.ps1 -KoreHost "127.0.0.1" -KorePort 2350 -ListenHost "127.0.0.1" -ListenPort 18085 -CommandToken "UM_TOKEN_LONGO_E_ALEATORIO"
```

Depois crie uma tarefa no **Task Scheduler** chamando:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File C:\openkore\tools\start-gateway.ps1
```
